...
# Load REGISTRATION_KEY from .env
...
# Create test engine with NullPool to ensure fresh connections per test
...
async def db_session() -> AsyncGenerator[AsyncSession, None]:
    ...
    async with TestSessionLocal() as session:
        yield session
        # Don't rollback - clean up is handled by setup_database (drop_all)

# Override the app's get_db dependency (NOT a pytest fixture - just a regular async generator function)
...
async def test_user(db_session: AsyncSession) -> User:
    ...
    db_session.add(user)
    await db_session.commit()  # Commit to make visible to all connections
...
async def authenticated_client_2(
    ...
    """Provide an authenticated test client for user 2."""
    transport = ASGITransport(app=app, raise_app_exceptions=True)
    async with AsyncClient(
        transport=transport,
        base_url="http://test",
